...
function processWorkflows($pdo, $userId, $conversationId, $contextData = []) {
    try {
        ...
        // Normalize event type if old format was passed
        if ($eventType === 'MESSAGE_RECEIVED') {
            // Keep as is, maps to KEYWORD trigger usually
        }
        ...
        foreach ($workflows as $wf) {
            ...
            log_debug("Checking Workflow {$wf['id']} ({$wf['name']}). Type: $triggerType vs Event: $eventType");
            ...
            elseif ($triggerType === 'CONVERSATION_CLOSED' && $eventType === 'CONVERSATION_CLOSED') {
                $shouldTrigger = true;
                log_debug("MATCH! Conversation Closed event.");
            }
            // 4. PAYMENT RECEIVED
            elseif ($triggerType === 'PAYMENT_RECEIVED' && $eventType === 'PAYMENT_RECEIVED') {
                $shouldTrigger = true;
                log_debug("MATCH! Payment Received event.");
            }
            // 5. TAG ADDED
            elseif ($triggerType === 'TAG_ADDED' && $eventType === 'TAG_ADDED') {
                $targetTag = trim($wf['keywords']); // For TAG_ADDED, keywords column stores the tag name
                $addedTag = trim($contextData['tag_name'] ?? '');

                if (strcasecmp($targetTag, $addedTag) === 0) {
                    $shouldTrigger = true;
                    log_debug("MATCH! Tag Added: '$addedTag'");
                }
            }
            ...
            if ($shouldTrigger) {
                ...
                // Break after first trigger? Usually yes to avoid conflicts.
                break;
            }
        }
