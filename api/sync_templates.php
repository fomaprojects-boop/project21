...
try {
    ...
    if ($http_code !== 200) {
        ...
        if ($meta_code == 190) {
            echo json_encode([
                'status' => 'auth_error',
                'message' => 'Your WhatsApp connection has expired or is invalid. Please reconnect in Settings > Channels.'
            ]);
            exit;
        }

        throw new Exception("Failed to fetch templates from Meta: " . $meta_message);
    }
