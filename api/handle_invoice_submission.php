<?php
// api/handle_invoice_submission.php
require_once 'db.php';
...
try {
    ...
    // (Inabaki kama mwanzo... tunapakia faili)
    $file_url = null;
    ...
    if ($use_existing == 'yes') {
        ...
        $stmt_update_request = $pdo->prepare(
            "UPDATE payout_requests SET 
                service_type = ?, amount = ?, invoice_url = ?, status = 'Submitted', submitted_at = CURRENT_TIMESTAMP,
                payment_method = ?, bank_name = ?, account_name = ?, account_number = ?, mobile_network = ?, mobile_phone = ?
             WHERE id = ?"
        );
        $stmt_update_request->execute([
            $service_type, $amount, $file_url,
        ...
        $stmt_update_request = $pdo->prepare(
            "UPDATE payout_requests SET 
                service_type = ?, amount = ?, payment_method = ?, bank_name = ?, account_name = ?, 
