...
class FinancialPDF extends TCPDF {
    ...
    public function Header() {
        ...
        if (!empty($this->logo_path) && file_exists($this->logo_path)) {
            $this->Image($this->logo_path, 10, 10, 25, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
             ...
             // Handle URL images if server configuration allows
        ...
        // Separator Line (Violet #7c3aed)
        $this->SetLineStyle(array('width' => 1, 'color' => array(124, 58, 237))); // RGB for #7c3aed
    ...
    public function Footer() {
        ...
        $this->SetTextColor(128, 128, 128); // Grey
...
try {
    ...
    if (!empty($logo_url) && !filter_var($logo_url, FILTER_VALIDATE_URL)) {
         ...
         if (file_exists($fs_path)) {
             $pdf->logo_path = $fs_path;
         } else {
             $pdf->logo_path = dirname(__DIR__) . '/assets/logo.png';
         }
    } else {
    ...
    $html .= row('Total Revenue', format_accounting_number($summary_data[$year]['total_revenue']), format_accounting_number($summary_data[$year - 1]['total_revenue']), false, false);
    ...
    $html .= row('Net Profit After Tax', format_accounting_number($summary_data[$year]['net_profit_after_tax']), format_accounting_number($summary_data[$year - 1]['net_profit_after_tax']), true, false);
    ...
    $html .= row('Non-Current Assets (Fixed)', format_accounting_number($balance_sheet_data[$year]['non_current_assets']), format_accounting_number($balance_sheet_data[$year - 1]['non_current_assets']), false, false);
