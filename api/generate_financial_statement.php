...
// Calculate Share Capital explicitly for the PDF note
$manual_inputs['share_capital'] = $manual_inputs['number_of_shares'] * $manual_inputs['par_value'];
...
try {
    ...
    $ppe_additions = $all_financial_data['ppe_additions_detail'] ?? ['current_year' => [], 'prev_year' => []];
    ...
    <ol>
        ...
        <br>
        <li><b>Revenue Recognition (IFRS 15 / TRA Compliance):</b> Revenue is recognized on a cash basis when payment is received and fiscal receipt is issued, aligning with EFD requirements. Unpaid invoices are recognized as Deferred Income.</li>
        <br>
        <li><b>Property, Plant and Equipment (IAS 16):</b> Items of property, plant and equipment are measured at cost less accumulated depreciation. Depreciation is calculated on a pro-rata basis.</li>
        <br>
        ...
        <br>
        <li><b>Taxation (IAS 12):</b> Current tax is the expected tax payable on the taxable income for the year. Tax assets are recognized for prepaid taxes during loss periods.</li>
    </ol>';
    ...
    // Payroll Liabilities
    $html .= row('Payroll Liabilities (PAYE, NSSF)', format_accounting_number($balance_sheet_data[$year]['payroll_liabilities'] ?? 0), format_accounting_number($balance_sheet_data[$year - 1]['payroll_liabilities'] ?? 0), false, true);

    $html .= row('Deferred Income (Unpaid Invoices)', format_accounting_number($balance_sheet_data[$year]['deferred_income'] ?? 0), format_accounting_number($balance_sheet_data[$year - 1]['deferred_income'] ?? 0), false, true);

    // Tax Payable (Income + VAT)
    $html .= row('Tax Payable (Income Tax & VAT) (Note 5)', format_accounting_number($balance_sheet_data[$year]['tax_payable']), format_accounting_number($balance_sheet_data[$year - 1]['tax_payable']));
    ...
    // Note 3: Revenue
    $html .= '<h3>Note 3: Revenue</h3><p>Revenue represents invoiced sales and services, recognized upon receipt of payment (Cash Basis), net of VAT.</p>';
    $html .= '<table cellpadding="5"><thead><tr><th width="70%">Description</th><th width="30%">Amount</th></tr></thead><tbody>';
    $html .= row('Sales / Services (Collected)', format_accounting_number($summary_data[$year]['total_revenue']), '', false, false);
    $html .= '</tbody></table>';
    ...
    $html .= row('Total Operating Expenses', format_accounting_number($summary_data[$year]['opex']), '', true, true);
    $html .= '</tbody></table>';

    // Note 5: Taxation
    $html .= '<h3>Note 5: Taxation</h3>';
    $tax_details = $summary_data[$year]['tax_computation'] ?? [];
    $html .= '<p>Taxation is calculated based on ' . ($tax_details['method'] ?? 'Standard') . '.</p>';
    if (!empty($tax_details['details'])) {
         $html .= '<p><b>Calculation Basis:</b> ' . htmlspecialchars($tax_details['details']) . '</p>';
    }
    $html .= '<table cellpadding="5"><thead><tr><th width="70%">Description</th><th width="30%">Amount</th></tr></thead><tbody>';
    $html .= row('Taxable Income (Profit Before Tax)', format_accounting_number($summary_data[$year]['profit_before_tax']), '', false, false);
    $html .= row('Tax Expense (' . ($tax_details['rate'] ?? '') . ')', format_accounting_number($summary_data[$year]['income_tax_expense']), '', true, false);

    // Detailed Breakdown of Liability
    $html .= '<tr><td colspan="2" style="font-weight:bold; background-color:#f3f4f6;">Liabilities Breakdown</td></tr>';
    $html .= row('Income Tax Liability', format_accounting_number($balance_sheet_data[$year]['income_tax_payable']), '', false, false);
    $html .= row('VAT Liability (Collected - Paid)', format_accounting_number($balance_sheet_data[$year]['vat_liability']), '', false, false);
    $html .= row('Total Tax Payable', format_accounting_number($balance_sheet_data[$year]['tax_payable']), '', true, true);
    $html .= '</tbody></table>';
    ...
    // PPE Additions Detail
    if (!empty($ppe_additions['current_year']) || !empty($ppe_additions['prev_year'])) {
        $html .= '<h4>Additions to Property, Plant and Equipment (Detail)</h4>';
        $html .= '<table cellpadding="5"><thead><tr><th width="50%">Item</th><th width="25%">' . $year . '</th><th width="25%">' . ($year - 1) . '</th></tr></thead><tbody>';

        $html .= '<tr><td colspan="3" style="font-weight:bold; background-color:#f3f4f6;">Current Year (' . $year . ')</td></tr>';
        if (empty($ppe_additions['current_year'])) {
             $html .= '<tr><td colspan="3">Nil</td></tr>';
        } else {
            foreach ($ppe_additions['current_year'] as $item) {
                $html .= '<tr>
                    <td style="border:1px solid #e5e7eb;">'.htmlspecialchars(ucfirst($item['name'])).' ('.htmlspecialchars($item['date_acquired']).')</td>
                    <td style="border:1px solid #e5e7eb; text-align:right;">'.format_accounting_number($item['cost']).'</td>
                    <td style="border:1px solid #e5e7eb; text-align:right;">-</td>
                </tr>';
            }
        }

        $html .= '<tr><td colspan="3" style="font-weight:bold; background-color:#f3f4f6;">Previous Year (' . ($year - 1) . ')</td></tr>';
        if (empty($ppe_additions['prev_year'])) {
             $html .= '<tr><td colspan="3">Nil</td></tr>';
        } else {
             foreach ($ppe_additions['prev_year'] as $item) {
                $html .= '<tr>
                    <td style="border:1px solid #e5e7eb;">'.htmlspecialchars(ucfirst($item['name'])).' ('.htmlspecialchars($item['date_acquired']).')</td>
                    <td style="border:1px solid #e5e7eb; text-align:right;">-</td>
                    <td style="border:1px solid #e5e7eb; text-align:right;">'.format_accounting_number($item['cost']).'</td>
    ...
    if (!empty($investment_details)) {
        ...
        foreach ($investment_details as $inv) {
            $html .= '<tr><td style="border:1px solid #e5e7eb;">' . htmlspecialchars($inv['description'] ?? 'Investment') . '</td><td style="border:1px solid #e5e7eb;">' . htmlspecialchars($inv['investment_type']) . '</td><td style="border:1px solid #e5e7eb;text-align:right;">' . format_accounting_number($inv['purchase_cost']) . '</td></tr>';
        }
        $html .= '</tbody></table>';
