...
function get_expense_breakdown_by_category($year) {
    ...
    // 1. Direct Expenses (Internal)
    // Rule: Claim + Approved -> Expense. Requisition + Paid -> Expense. Requisition + Approved -> AP (Liability, not P&L Expense yet? Or Accrued Expense?)
    // User Instructions: "Refactor Income Statement... COGS... OPEX".
    // "Status 'Approved' OR 'Paid' -> Treat as EXPENSE" (Payouts).
    // "Requisition: Status 'Paid' -> Treat as EXPENSE". "Status 'Approved' -> Treat as AP".
    // Implication: Unpaid Requisitions are NOT in the Income Statement Expense line yet?
    // If we follow this strictly:

    $expenses = [];

    // A. Requisitions: Only 'Paid' counts as Expense
    $stmt_req = $pdo->prepare("SELECT amount, expense_type FROM direct_expenses WHERE YEAR(date) = ? AND type = 'requisition' AND status = 'Paid'");
    $stmt_req->execute([$year]);
    $expenses = array_merge($expenses, $stmt_req->fetchAll(PDO::FETCH_ASSOC));

    // B. Claims: 'Approved' counts as Expense (Immediate Cash Out)
    // Assuming 'Paid' claims also count if status exists
    $stmt_claim = $pdo->prepare("SELECT amount, expense_type FROM direct_expenses WHERE YEAR(date) = ? AND type = 'claim' AND status IN ('Approved', 'Paid')");
    $stmt_claim->execute([$year]);
    $expenses = array_merge($expenses, $stmt_claim->fetchAll(PDO::FETCH_ASSOC));

    // 2. Vendor Payouts
    // Rule: 'Approved' OR 'Paid' -> Treat as EXPENSE
    $stmt_payout = $pdo->prepare("SELECT amount, service_type as expense_type FROM payout_requests WHERE YEAR(processed_at) = ? AND status IN ('Approved', 'Paid')");
    ...
    // COGS Types
    // Payouts: Goods, Goods/Products, Materials, Production
    // Direct Exp: materials, production_cost
    $cogs_keywords_direct = ['materials', 'production_cost'];
    $cogs_keywords_payout = ['goods', 'goods/products', 'materials', 'production'];

    // Process Direct Expenses
    foreach ($expenses as $exp) {
        $type = strtolower(trim($exp['expense_type']));
        if (in_array($type, $cogs_keywords_direct)) {
            $cogs += $exp['amount'];
        } else {
            $opex += $exp['amount'];
        }
    ...
    foreach ($payouts as $pay) {
        $type = strtolower(trim($pay['expense_type']));
        if (in_array($type, $cogs_keywords_payout)) {
            $cogs += $pay['amount'];
        } else {
            $opex += $pay['amount'];
        }
...
function calculate_ap_balance($year) {
    global $pdo;
    // AP = Liability (Committed but not Paid)

    // 1. Vendor Payouts
    // Rule: Status 'Submitted' -> AP.
    // Note: 'Approved' moves to Expense/Cash Out logic, so it leaves AP?
    // Usually Approved but Unpaid is AP. But user logic says "Approved implies payment process is cleared".
    // So strictly 'Submitted'.
    $stmt_pay = $pdo->prepare("SELECT SUM(amount) FROM payout_requests WHERE YEAR(submitted_at) <= ? AND status = 'Submitted'");
    $stmt_pay->execute([$year]);
    $ap_payouts = $stmt_pay->fetchColumn() ?: 0;

    // 2. Internal Expenses (Requisitions)
    // Rule: Type 'Requisition' AND Status 'Approved' -> AP.
    $stmt_exp = $pdo->prepare("SELECT SUM(amount) FROM direct_expenses WHERE YEAR(date) <= ? AND type = 'requisition' AND status = 'Approved'");
    $stmt_exp->execute([$year]);
    $ap_expenses = $stmt_exp->fetchColumn() ?: 0;
