...
function calculate_base_financials($target_year) {
    ...
    foreach ($years as $y) {
        ...
        $total_tax_paid_cash = calculate_tax_paid($y);

        $vat_paid = calculate_tax_paid($y, 'VAT');
        $vat_liability = calculate_vat_liability($y);

        $income_tax_paid = $total_tax_paid_cash - $vat_paid;
        $income_tax_liability = $tax_expense - $income_tax_paid;
...
function calculate_payroll_liabilities($year) {
    ...
    // Liability = (Gross - Net)
    // Assuming no explicit payment of these liabilities is recorded in the system yet.
...
function calculate_vat_liability($year) {
    ...
    // VAT Paid (via tax_payments table where type = 'VAT')
    $vat_paid = 0;
    try {
        $stmt2 = $pdo->prepare("SELECT SUM(amount) FROM tax_payments WHERE tax_type = 'VAT' AND YEAR(payment_date) <= ?");
        $stmt2->execute([$year]);
...
function calculate_tax_paid($year, $type = null) {
    global $pdo;
    $sql = "SELECT SUM(amount) FROM tax_payments WHERE YEAR(payment_date) = ?";
    $params = [$year];
